...
.Run Spring examples for caching
[source,bash,options="nowrap",subs=attributes+]
----
$ {package_exec}@spring-caching <1>
$ {package_exec}@spring-annotations <2>
----
<1> Tests caching
<2> Tests annotations

.Run Spring examples for session
...
Then navigate to `http://localhost:8080/session` and observe session added to the Infinispan cluster.
